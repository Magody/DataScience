...
class HandDetector():
    ...
    def find_hands(self, img, draw=True, flipType=True):
        """Detects and draw the hands landmarks

        Args:
            img (np.matrix): Image got by cv2
            draw (bool, optional): If draw in the img or not. Defaults to True.
...
def main():
    ...
    detector:HandDetector = HandDetector(min_detection_confidence=0.8, max_num_hands=2)
    while True:
        # Get image frame
