...
class Neat:
    ...
    def reset(self, input_size:int, output_size:int, population:int)->None:
        ...
        for i in range(self.POPULATION):
            genome:Genome = self.empty_genome(connect_input_output=False)
    ...
    def mutate(self)->None:
        ...
        for i in range(len(self.genomes.data)):
            ...
            if p4 < genome.mutation_rates["toggle"]:
                genome.mutateRandomLinkToggle()
    ...
    def mutateGenomeNode(self, genome:Genome):
        ...
        if replace_index > 0:
            if not genome.existNeuron(replace_index):
