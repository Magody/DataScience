...
class Population:
    ...
    def fitness_function(self, target: list) -> list:
        ...
        for index_individual in range(self.max_population):
            ...
            sum_correct_characters = 0

            for index, gen in enumerate(self.chromosomes[index_individual].gens):
                # in this problem a gen is a character
                if gen == target[index]:
                    sum_correct_characters += 1

            fitness[index_individual] = sum_correct_characters
    ...
    @staticmethod
    def has_reached_the_top(fitness: list, extra: list) -> bool:
        mean_fitness = (sum(fitness)/len(fitness))/len(extra)
        bool_value = mean_fitness >= Population.THRESHOLD_SIMILARITY
